...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 人事变动 <span class="c-gray en">&gt;</span> 人事变动记录 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20">
<form action="">
		<div class="text-c">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach var="i" begin="1" end="${employees.size() }">
			<tr class="text-c">
				<td><input type="checkbox" value="${employees.get(i-1).employeeId }" name="checkRecord"></td>
				...
				<td><u style="cursor:pointer" title="查看信息" class="text-primary" 
					onclick="member_show('${employees.get(i-1).employeeName }','employee-show.jsp','${employees.get(i-1).employeeId }','360','400')">
					<span class="label label-waring radius">${employees.get(i-1).employeeName }</span></u></td>
				</c:otherwise>
				</c:choose>
				<td>${employees.get(i-1).employeeSex }</td>
				<td>${employees.get(i-1).employeePhone }</td>
				<td>${employees.get(i-1).employeeEmail }</td>
				<td class="text-l">${employees.get(i-1).employeeAddress }</td>
				<td>${employees.get(i-1).employeeEnterTime }</td>
				<td>${employees.get(i-1).employeeLeaveTime }</td>
				<c:choose>
				<c:when test="${employees.get(i-1).employeeStatus == '工作中'}">
				<td class="td-status"><span class="label label-success radius">${employees.get(i-1).employeeStatus }</span></td>
				</c:when>
				<c:otherwise>
				<td class="td-status"><span class="label label-waring radius">${employees.get(i-1).employeeStatus }</span></td>
				</c:otherwise>
				</c:choose>
				<td class="td-manage"> </a> <a title="删除" href="javascript:;" onclick="member_del(this,'1')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
			</c:forEach>
